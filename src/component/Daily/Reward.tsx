...
const Reward = () => {
  ...
  const signTypedData = async () => {
    try {
      ...
      const dailyAttendance = new Contract(
        process.env.REACT_APP_CONTRACT_DAILY_ATTENDANCE,
        DailyAttendanceAbi,
        provider
      );
      const forwarder = new Contract(
        process.env.REACT_APP_CONTRACT_FORWARDER,
        ForwarderAbi,
        provider
      );
      ...
      const alreadyCheckedIn = await dailyAttendance.checkedInToday(
        walletAddress
      );
      if (alreadyCheckedIn) {
      ...
      const message = {
        ...
        data: dailyAttendance.interface.encodeFunctionData(
          "checkIn",
          undefined
        ),
      };
      ...
      const res = await API.post(
        process.env.REACT_APP_API_PERSONAL_CHECK,
        param,
        {
          headers: {
  ...
  useEffect(() => {
    const fetchReceivedStatus = async () => {
      try {
        const response: MyTotalLoginsResponse = await ApiDaily.myTotalLogin(
          walletAddress
        );
        const data = {
          ...
          style={{
            cursor:
              isLoggedIn && isTodayCheckAvailable(index)
                ...
                : "default",
            backgroundImage:
              isLoggedIn && isTodayCheckAvailable(index) ? mega8 : mg8gray,
